...
function leadTaskNotes(l) {
  ...
  lines.push('', 'Generated by The A&R Team.', 'Lead ref: ' + l.artistKey);
  return lines.join('\n');
}
// The ref a task carries — the explicit line, or (tasks from before it existed) derived from
// the contact lines with the same precedence the canonical key uses: email, handle, name.
function parseLeadRef(notes) {
  const n = notes || '';
  let m = /^Lead ref: (.+?)\s*$/m.exec(n);   // keys may contain spaces (n:big room)
  if (m) return m[1];
  m = /^Email: (\S+@\S+)$/m.exec(n); if (m) return 'e:' + m[1].toLowerCase();
  m = /^Instagram: @?([A-Za-z0-9_.]+)$/m.exec(n); if (m) return 'i:' + m[1].toLowerCase();
  m = /^Artist: (.+)$/m.exec(n); if (m && m[1].trim() !== '—' && chartKey(m[1])) return 'n:' + chartKey(m[1]);
  return null;
}
// Reconcile the ledger with the project as it actually is (the first press of a run):
//  - two or more tasks carrying one ref → keep the newest, delete the rest (they are ours);
//  - a task with a ref and no ledger row → adopt it (a row with no record, so the next write
//    refreshes it); a row whose task is gone → mark it 'deleted' so the lead is NEVER
//    recreated (the operator removed it on purpose).
// This is what makes the feature self-healing after the two-loop race of 2026-09-19, and it
...
async function reconcileLeadsProject(proj, { deadline, mark, out }) {
  ...
  const byRef = new Map();
  for (const t of tasks) { const ref = parseLeadRef(t.notes); if (!ref) continue; (byRef.get(ref) || byRef.set(ref, []).get(ref)).push(t); }
  ...
  for (const [ref, list] of byRef) {
    ...
    for (const dup of list.slice(1)) {
      ...
      try { await asanaFetch(`/tasks/${dup.gid}`, { method: 'DELETE' }); } catch (e) { if (!/Asana 404/.test(e.message)) { mark('reconcile: delete FAILED ' + e.message); continue; } }
      present.delete(dup.gid);
      out.merged++; mark('reconcile: removed duplicate ' + dup.gid + ' (' + ref + ')');
    }
    const row = ledger.get(ref);
    if (!row) {
      await db.run('INSERT INTO asana_leads (artist_key, task_gid, round_id, score, played_day, synced_at) VALUES (?, ?, NULL, NULL, NULL, ?) ON CONFLICT (artist_key) DO NOTHING', [ref, keep.gid, now()]);
    } else if (row.task_gid !== keep.gid) {
      await db.run('UPDATE asana_leads SET task_gid = ?, round_id = NULL WHERE artist_key = ?', [keep.gid, ref]);
    }
  }
  // An EMPTY project is a fresh one (the setting was just pointed at it), not a project the
  // operator emptied by hand — nothing is marked deleted off it.
  for (const row of (tasks.length ? ledger.values() : [])) {
    if (row.task_gid !== 'deleted' && !present.has(row.task_gid) && !byRef.has(row.artist_key)) {
      await db.run("UPDATE asana_leads SET task_gid = 'deleted' WHERE artist_key = ?", [row.artist_key]);
      mark('reconcile: task gone, ' + row.artist_key + ' stays out');
    }
  }
  out.reconciled = complete;
  return complete;
}
...
async function syncLeadsToAsana({ pct, minVotes, limit = LEADS_BATCH, budgetMs = LEADS_BUDGET_MS, trace = [], reconcile = false } = {}) {
  const t0 = Date.now();
  ...
  const out = { ok: true, project: proj.gid, url: proj.url, fieldsError: proj.fieldsError,
    created: 0, updated: 0, skipped: 0, merged: 0, remaining: 0, failed: [], reconciled: null };
  if (reconcile) {
    const done = await reconcileLeadsProject(proj, { deadline, mark, out });
    if (!done) { mark('reconcile: out of time, the rest next press'); out.remaining = 1; out.trace = trace; return out; }
  }
  const data = await salesLeadsData({ pct, minVotes });
  mark('leads ready ' + data.leads.length);
  const ledger = new Map((await db.all('SELECT * FROM asana_leads', [])).map(r => [r.artist_key, r]));
  mark('ledger read ' + ledger.size);
  Object.assign(out, { leads: data.leads.length, cut: data.cut, total: data.total, pct: data.pct, minVotes: data.minVotes, excluded: data.excluded });
  let budget = limit;
  const fieldsFor = (l) => {
    const cf = {};
  ...
  for (const l of data.leads) {
    const rows = leadLedgerRows(ledger, l);
    // A task the operator deleted in Asana stays deleted: the lead is skipped for good.
    if (rows.some(r => r.task_gid === 'deleted')) { out.skipped++; continue; }
    const [row, ...extras] = rows;
    ...
    try {
      ...
      if (gid) {
        try { await asanaJson(`/tasks/${gid}`, 'PUT', body); out.updated++; }
          ...
          if (!/Asana 404/.test(e.message)) throw e;
          // Deleted in Asana by the operator: honour it, never recreate.
          await db.run("UPDATE asana_leads SET task_gid = 'deleted' WHERE artist_key = ?", [row.artist_key]);
          mark('write #' + l.rank + ' task gone, stays out'); out.skipped++; continue;
        }
      }
      if (!gid) {
...
async function handleApi(req, res, url) {
  ...
  if (p === '/api/admin/settings' && method === 'POST') {
    ...
    if ('asanaLeadsProject' in body) {
      const next = (body.asanaLeadsProject || '').toString().trim().replace(/\D/g, '').slice(0, 30) || null;
      const prev = (await db.get("SELECT v FROM settings WHERE k = 'asana_leads_project'"))?.v || null;
      await setOrClear('asana_leads_project', next);
  ...
  if (p === '/api/admin/leads/asana' && method === 'POST') {
    ...
    try {
      trace.push('0ms body');
      const { pct, minVotes, reconcile } = await readBody(req);
      trace.push('body read');
      // ONE press at a time, platform-wide. Two loops running together (a forgotten tab plus
      // a fresh one, 2026-09-19) each read the ledger before the other wrote it and doubled
      // the project. The lock is a settings row holding its expiry; the claim is a conditional
      // UPDATE so two instances cannot both win it (epoch ms are 13 digits, so text compares).
      ...
      if (!claim.changes) { clearTimeout(timer); return bad(res, 'A sync is already running (another tab or window). Wait for it to finish, then press again.', 409); }
      let r;
      try { r = await Promise.race([syncLeadsToAsana({ pct, minVotes, trace, reconcile: !!reconcile }), cutoff]); }
      finally { if (!r || !r.timedOut) await db.run("UPDATE settings SET v = '0' WHERE k = 'asana_leads_lock' AND v = ?", [lockUntil]); }
      clearTimeout(timer);
      if (r.timedOut) {
