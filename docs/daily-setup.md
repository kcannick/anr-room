...
## 6b. `RESULTS_CALLBACK_URL` + `RESULTS_CALLBACK_TOKEN` — optional, and silent until both are set

At the publish the app can tell makinitmag.com what happened to each record — rated, not
playable, or never rated, with the rating count and the room average — so the artist's status
page can **confirm** a review instead of promising one, and so a record that sat behind a dead
link is not counted as reviewed.

**Both must be set or nothing is attempted.** That is deliberate: a preview deployment must not
be able to post into production Drupal. It also means the feature is easy to forget — it is
built and deployed and will simply never fire until these exist.

- `RESULTS_CALLBACK_URL` — `https://makinitmag.com/api/anr-meeting/results`
- `RESULTS_CALLBACK_TOKEN` — **generated by the makinitmag side, not here.** Ask them for it;
  they send it as `X-ANR-Key` on their end and it must match.

Do not set these until the makinitmag endpoint actually exists. Posting to a URL that 404s is
handled (we treat their 404 as terminal and stop), but you would be marking days as settled
against an endpoint that never received anything.

**How to tell it worked.** Each day's session row carries `results_status`:

| Value | Meaning |
|---|---|
| `NULL` | never attempted — the two vars are unset, which is the normal state today |
| `sent` | delivered, they returned 200 |
| `unknown_day` | they returned 404 — the day is unknown on their side. Terminal; retrying cannot fix it |
| `failed` | gave up after ~2 hours of retries. Something is down or wrong |
