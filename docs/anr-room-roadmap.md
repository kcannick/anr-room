...
| 3.5 | **Notify-on-go-live** | code | 🟡 **PHASE 0 SHIPPED** (2026-07-01) — SMS (Twilio) + email fan-out when a session flips to live. Audience = that session's verified participants (registration = consent basis for the go-live notice; SMS additionally gated on `sms_marketing_consent`). Idempotent + audited via `notification_log` (015); inline concurrency-limited + capped (800) fan-out — moves to a queued drain at larger scale. `sms.js` mirrors `email.js` (console stub → `SMS_PROVIDER=twilio` + `TWILIO_*`). **Web Push deferred** → gated on the PWA shell / branding pin. iOS covered by SMS/email meanwhile. |
...
| 4.4 | **Live push via Ably (managed)** | code/integration | The real-time layer for the liveness feed + counters. **Managed service, nothing to run** — your server publishes events, clients subscribe. Free under 200 concurrent; ~$65/mo at 1,000. Replaces the old "migrate to SSE" plan. Build when liveness ships. |
...
| 6.4 | **Auto-seed A&R Wars from series close** | product/code | 2.0 upgrade: closing a series auto-creates an A&R Wars (binary) session pre-seeded with the qualifying top-N. Hooks already exist (leaderboard returns qualifiers; close is the trigger). Snaps onto current structure — no rework. Manual setup until then. |
