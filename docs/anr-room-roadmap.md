...
| 3.2 | **Binary poll full build** | code | ✅ **SHIPPED.** All 6 spec steps (`binary-poll-build-spec.md`) live: migration `003_binary_poll`, scoring (`roomSplitA`/`splitPoints`/`splitTier`/`rankBinaryVotes`), server (vote/round/ratify/state/export branches + sealed live-split preview), player UI (two-step Pick → Read-the-Room + reveal), admin UI (poll-type toggle, A/B entry, split tally, "Versus" badge), export (CSV+JSON binary columns). Overlay, recap (in-app + email, no 0–9 grade), and Score Card all handle binary; Top 8 Songs + Song Report intentionally excluded for Versus (409-enforced). Full binary coverage across all three suites (373 green). Substrate for online A&R Wars. |
...
| 6.4 | **A&R Wars tournament (bracket layer)** | product/code | **Format (operator, 2026-07-01):** online **single-elimination** tournament, **8 competitors** = curated mix (e.g. top 4 A&Rs from the month's series board + 4 invited Tastemakers — partly seeded, partly hand-picked; NOT a pure auto-seed). Competitors scout songs from a given **"service pack"** (zip) and play their picks head-to-head. Each matchup = a **binary (Versus) poll round** (Song A vs Song B); the **A&R Room audience votes**, majority side wins, winner advances. 8→4→2→1 = 7 matchups / 3 rounds. **Competitors are NOT session participants** — the regular audience does the voting. **What exists:** binary A/B voting + majority-winner (Versus). **DECISION (operator, 2026-07-01): runs on EXISTING features — no new build, NO bracket layer.** The service-pack zip + competitors' song-picks happen **outside the app** (host just enters the two songs per matchup via the existing binary "Queue a matchup" flow); viewers **earn points toward the current month's series**, so the Wars session is just **tagged into the active series** like any other. A roster/visual-bracket/auto-advancement/overlay-bracket layer was explicitly declined — host tracks the bracket manually. Nothing to build; item is effectively closed. |
