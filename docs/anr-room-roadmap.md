...
| 4.4 | **Live push via Ably (managed)** | code/integration | 🟡 **PHASE 1 SHIPPED** (2026-07-01) — `realtime.js` (key-gated, no-op without `ABLY_API_KEY`; stateless REST, serverless-safe). Server publishes a `change` signal per session on every material mutation (round open/close/ratify/extend/reopen/edit, go-live, broadcast); `/api/ably/token` mints subscribe-only tokens (key never leaves the server). **play.html + overlay.html** subscribe and refresh instantly on push (~0.8–1.4s verified), dropping the poll to a 15s heartbeat when connected and **auto-reverting to 2–2.5s polling** if disabled/disconnected — zero regression. Vote-count pushes intentionally skipped (hot path). **Follow-ups:** admin.html + home.html clients; push the actual leaderboard payload (make board compute O(1) at celebrity scale) + a short cache. |
